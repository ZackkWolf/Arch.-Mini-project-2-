...
void move(int x, int y)
{
    ...
    while (1)
    {
		...
		if(cc=='S' && read_pixel(x,y+7)==0 && read_pixel(x-7,y+7)==0 && read_pixel(x+7,y+7)==0){
			cc='S';
            clear_square(x, y);
        ...
        }else if (c == 0x71) // move left max
        {
		
            clear_square(x, y);
            if (read_pixel(x-7, y) == 0x0000 && read_pixel(x-7,y+7)==0 && read_pixel(x-7,y-7)==0)
            {
